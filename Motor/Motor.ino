...
void FollowBlock(int trackedBlock)
{
  ...
  leftMotor.writeMicroseconds(leftSpeed + 1500);
  rightMotor.writeMicroseconds(rightSpeed + 1500);
}
...
void ScanForBlocks()
{
  if (millis() - lastMove > 20)
  {
    ...
    if ((panLoop.m_pos >= RCS_MAX_POS)||(panLoop.m_pos <= RCS_MIN_POS))
    {
      ...
      if (scanIncrement < 0)
      {
        leftMotor.write(1500-250);
        rightMotor.write(1500+250);
      }
      else
      {
        motors.setLeftSpeed(1500+180);
        motors.setRightSpeed(1500-180);
      }
